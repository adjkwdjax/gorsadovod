...
- `DELETE /api/articles/{slug}/`
    - вход: `slug` (+ авторизация)
    - ограничения: удалить статью может только ее автор
...
- `GET /api/calendar/`
    - вход: ничего
    - выход: объект с полями `results` (сезонные работы) и `canEdit` (может ли текущий пользователь редактировать календарь)
- `PUT /api/calendar/`
    - вход: `results` (массив сезонов с полями `season`, `months`, `tasks`)
    - доступ: только администратор (`is_staff`)
    - выход: обновленный объект календаря с полями `results` и `canEdit`
...
- `GET /api/clubs/{id}/messages/`
    - вход: `id` клуба (+ авторизация + членство в клубе)
    - выход: сообщения общего чата клуба
- `POST /api/clubs/{id}/messages/`
    - вход: `content` (+ авторизация + членство в клубе)
    - выход: отправленное сообщение общего чата
- `DELETE /api/clubs/{id}/messages/{messageId}/?scope=all`
    - вход: `id` клуба, `messageId` (+ авторизация + членство в клубе)
    - ограничения: удалить можно только свое сообщение
    - выход: `204 No Content`
...
- `DELETE /api/messages/{userId}/{messageId}/`
    - вход: `userId`, `messageId` (+ авторизация)
    - поведение: удаляет только свое сообщение для текущего пользователя
    - ограничения: удалить можно только сообщение, где текущий пользователь отправитель
    - доп. режим: `?scope=all` удаляет сообщение у всех участников диалога
- `DELETE /api/messages/{userId}/`
    - вход: `userId` (+ авторизация)
    - поведение: удаляет всю переписку в выбранном диалоге только для текущего пользователя
    - выход: `{ "deletedCount": number }`
