...
### календарь и категории (пока заглушки)
...
примечание: во `views.py` есть еще функции для деталей растения и добавления записи, но в `urls.py` они пока не подключены.
